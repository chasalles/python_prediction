...
df = pd.read_csv('Dados\\PETR3-DIARIO-MAXIMO.csv')
...
conv_model = tf.keras.Sequential([
    tf.keras.layers.Conv2D(filters=32, kernel_size=(CONV_WIDTH,), activation='relu'),
...
arquivo = open("ArquivosGerados\\resultado_maxima.csv", "a")

for index, row in df_teste_previsao.iterrows():
    arquivo.write("CONV-MAX;" + str(row["tempo"]) + ";" + str(row["previsao"]) + ";" + str(row["preditos"]) + "\n")
